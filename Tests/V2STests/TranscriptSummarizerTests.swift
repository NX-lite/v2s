import Testing
@testable import v2s

@Suite struct TranscriptSummarizerTests {
    @Test func extractiveSummarySelectsRepeatedTopicAndKeepsTranscriptOrder() throws {
        let transcript = """
        The product launch was delayed by heavy rain. The cafeteria introduced a new lunch menu. \
        Engineers rescheduled the product launch for Tuesday. Safety teams approved the Tuesday launch plan.
        """

        let summary = TranscriptSummarizer.extractiveSummary(text: transcript, languageID: "en")

        #expect(summary.contains("product launch"))
        #expect(summary.contains("Tuesday"))
        #expect(!summary.contains("cafeteria"))
        let productLaunch = try #require(summary.range(of: "product launch"))
        let tuesday = try #require(summary.range(of: "Tuesday"))
        #expect(productLaunch.lowerBound < tuesday.lowerBound)
    }

    @Test func extractiveSummaryReturnsShortTranscriptUnchanged() {
        let transcript = "A single complete thought."

        #expect(TranscriptSummarizer.extractiveSummary(text: transcript, languageID: "en") == transcript)
    }

    @Test func extractiveSummaryPreservesSourceLanguage() {
        let transcript = """
        新产品将在周二发布。团队今天完成了安全检查。发布计划已经获得批准。天气可能影响周二的发布活动。
        """

        let summary = TranscriptSummarizer.extractiveSummary(text: transcript, languageID: "zh-Hans")

        #expect(!summary.isEmpty)
        // CJK sentences carry their own full-width terminator, so joining them
        // must not introduce spaces that never appeared in the transcript.
        #expect(!summary.contains(" "))
        for sentence in summary.split(separator: "\u{3002}") {
            #expect(transcript.contains(sentence))
        }
    }

    @Test func extractiveSummarySeparatesLatinSentencesWithSpace() {
        let transcript = """
        The launch slipped a week. Rain fell hard all morning. \
        Engineers moved the launch to Tuesday. Safety approved the Tuesday launch.
        """

        let summary = TranscriptSummarizer.extractiveSummary(text: transcript, languageID: "en")

        #expect(summary.contains("Tuesday"))
        #expect(summary.contains(". "))
    }

    @Test func extractiveSummaryDoesNotAddSpacesAfterFullWidthCJKTerminators() {
        let transcript = "新产品将在周二发布！团队今天完成了安全检查？发布计划已经获得批准。天气可能影响周二的发布活动！"

        let summary = TranscriptSummarizer.extractiveSummary(text: transcript, languageID: "zh-Hans")

        #expect(!summary.contains("！ "))
        #expect(!summary.contains("？ "))
    }
}
